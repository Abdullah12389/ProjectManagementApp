...
export default function MultipleContainers({titles,className}:MutipleContainerProp){
    const { tasks:data }=useKanbanData();
    ...
    function handleDragEnd(event:DragEndEvent){
        ...
        if(!over){
            ...
            api.patch(`/task/${activeId}/status/${over.id}`).then(()=>{
                    setTasks(prev=>{
                        if(!prev) return prev;
                        const temp=[...prev];
...
function NewTask({open,setOpen}:{open:boolean,setOpen:any}){
    const { project,users,reload}=useKanbanData();
    ...
    const setData=(field:string,value:any)=>setDataState(prev=>({...prev,[field]:value}))
    const submit=(e:React.FormEvent<HTMLFormElement>)=>{
        e.preventDefault()
        setProcessing(true)
        api.post('/task',data).then(async()=>{
            setOpen(false)
            await reload()
