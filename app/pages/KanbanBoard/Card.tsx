...
import { Box, IconButton, Menu, MenuItem, Card, CardHeader, Typography, CardContent } from "@mui/material";
...
import { useKanbanData } from "./KanbanData";
...
export default function Cards({ tasks, OverId }: { className?: string; tasks: Task[]; OverId: any }) {
    ...
    const data = tasks.filter((task) => task.title.toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase()));

    return (
        <SortableContext strategy={verticalListSortingStrategy} items={tasks.map((task) => task.id)}>
                ...
                {data.map((task) => (
                    <Box key={task.id}>
                        <Slide direction="up" cascade damping={0.2} triggerOnce>
                            <TaskCard task={task} className="rounded-2xl!" />
                        </Slide>
                        {OverId === task.id && <Box className="text-blue-300">___________________________________</Box>}
                    </Box>
                ))}
            </Box>
        </SortableContext>
...
function TaskMenu({ id }: { id: number }) {
    const { reload } = useKanbanData();
    const [anchorElement, setAnchorElement] = useState<HTMLElement | null>(null);

    function deleteit() {
        if (confirm("Do you really wanna proceed")) {
            void api.delete(`/task/${id}`).then(reload);
        }
    }
...
function AssignedMenu({ names }: { names: { id: number; name: string }[] }) {
    ...
    return (
        <Box>
            ...
            <Menu open={Boolean(anchorEl)} anchorEl={anchorEl} onClose={() => setAnchorElement(null)}>
                {(names || []).map((name) => <MenuItem key={name.id}>{name.name}</MenuItem>)}
            </Menu>
        </Box>
    );
}
...
export function TaskCard({ task, className }: { task: Task; className: string }) {
    ...
    return (
        <Card ref={setNodeRef} className={className} sx={{ background: "linear-gradient(135deg, #1a1a2e, #2c2c54, #6a0dad)" }}>
            <CardHeader title={<Typography variant="h5" {...attributes} {...listeners}>{task.title}</Typography>} subheader={String(task.due_date).slice(0, 10)} action={<TaskMenu id={task.id} />} />
            <CardContent>
                <Box className="flex justify-between">
                    <AssignedMenu names={task.user} />
                    <ChatBubbleOvalLeftEllipsisIcon onClick={() => visit(`/task/${task.id}`)} className="w-10 h-10" aria-label="comment" />
                </Box>
            </CardContent>
        </Card>
    );
